...
- **`--subjects "<주제>"` (선택)**: 사용자가 시리즈 주제를 주면 시트의 샘플
  소재들이 그 주제와 관련되게 생성된다 (프롬프트에 SUBJECT THEMES 한 줄만
  추가됨). 클립 생성 시 시트의 샘플이 소재로 유출되는 성질을 역이용하는
  것이다. 미지정 시 기존과 완전히 동일. 주제를 심은 시트는
  `style_reference_<시리즈>.png`처럼 별도 이름으로 저장해 무주제 정본과
  구분한다
