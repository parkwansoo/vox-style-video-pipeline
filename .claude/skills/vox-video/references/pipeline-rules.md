...
> 이 절에는 원래 "제품 스펙 문서의 수치가 실측과 달라 숫자가 방해된다"고
> 적혀 있었으나 **틀린 근거였다.** 기울어진 제품 사진을 재서 1.51이 나왔던
> 것이고, 왜곡 없는 사진 실측은 1.74로 문서 수치(1.7)와 맞았다. 숫자를 빼는
> 이유는 값이 틀려서가 아니라 프롬프트 입력으로 작동하지 않기 때문이다.

**프롬프트에 레퍼런스 사진의 촬영 각도를 언급하지 않는다.** 어떤 레퍼런스를
고를지는 자산 준비 단계의 판단이고(각 제품 폴더의 `SOURCE.md`에 기록), 생성
프롬프트에 들어가면 **결과물의 구도까지 그 각도로 끌려간다.** 클립의 카메라
각도는 위의 "프레이밍 선택"이 따로 정한다.
