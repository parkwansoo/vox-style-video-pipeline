...
- [ ] 2026-08-03 **무음 압축을 tts.py의 ASR 직전으로 이동** (핵심) — 3영상 모두 실발화 55~57%/무음 27~32%로 일관된 TTS 구조 문제. 압축 후에도 정렬률 1.0000/0.9118로 무해함을 실측 확인. `words.json`이 자동으로 압축 기준이 되어 뒤 단계는 무변경. 배속이 거의 불필요해진다(최대 1.89x→불요). 상세는 PROGRESS.md 2026-08-03 절 — scripts/tts.py
- [ ] 2026-08-03 **무음 압축은 대본 기반으로 문장 경계를 구분한다** (균일 클램프 금지, 사용자 지시) — 문장 경계(마침표·물음표 뒤) 0.45초 / 문장 내부 호흡 0.25초 / 0.2초 미만은 무보정. auto-editor 조사 결과 그쪽도 볼륨 고정 임계값(threshold=0.04)이라 문장 경계를 모른다. 우리는 대본+words.json이 있어 판정 가능 — 이게 도구를 안 쓰는 결정적 이유. auto-editor에서 차용할 것: 비대칭 마진(`--margin 앞,뒤`), mincut(짧은 무음 무보정), 무음 배속(`--set-action speed`, 제거 대신 빠르게 넘기기) — scripts/tts.py
