...
- [ ] 2026-08-01 Gemini TTS 한국어 보이스 청감 비교 (**기본은 Charon**, 30종 전부 사용 가능) 후 기본값 확정 — .env `VOX_TTS_VOICE` 또는 `tts.py --voice`. 2026-08-03 확인: 백엔드에 30종(남 12/여 18) 전부 등록돼 있어 바로 비교 가능하다. 성별·결 분류는 SKILL.md 3단계 표 참조. ※기존 기록의 "Kore 기본 / GEMINI_TTS_VOICE"는 v0.3.0에서 clone-voice 백엔드로 갈아타기 전 정보라 실제와 달랐다 (정정 완료)
...
- [x] 2026-08-02 자막·타이틀 레이어를 Remotion으로 얹기 → **2026-08-04 구현 완료** (subtitler/ 자체 렌더러 + 7.5 단계, 20번 데이터 계약 호환. 상세: docs/설계-리모션-자막레이어.md)
...
- [x] 2026-08-04 vgshot SPEC.md에도 클래스 선입견 교정 패턴 적용 검토 → 같은 날 적용·검증 완료 (신버전 4표본 1.70~2.03, 구 실패모드 1.41~1.57 재발 0회)
- [ ] 2026-08-04 gen_video.py·assemble.py 기본값이 가로(16:9/1280x720)라 세로 작업에서 옵션 누락 실수 2회 발생 — 런 설정에서 비율을 읽거나 기본값을 바꾸는 방지책 검토 (PROGRESS 2026-08-04 교체 절 참조)
- [ ] 2026-08-04 segment.mjs 개선분(~라 연결어미·관형형·고아 억제·폭 기반 분할)을 20번 숏츠자동화에 역기여 검토 + 20번 defaultProps 잔여값 함정 점검 권고 (설계서 v2 '역기여 후보' 절)
- [ ] 2026-08-04 20번 outputs/audio/vox0001-후보1 합성 묶음(자막 테스트 잔여물) 정리 — 사용자 확인 후 삭제 (그쪽 git 추적 폴더)
