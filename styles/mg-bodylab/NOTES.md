...
- **프레이밍**: DNA의 components에 "centred to camera"가 박혀 있어 방치하면
  전 클립이 정면으로 나온다. 인물 컷과 훅은 HERO LOW / SCALE CONTRAST를
  우선하고, 해부 단면·층 설명 컷은 SPEC FRONT를 유지한다
  (pipeline-rules.md의 "프레이밍 선택" 참조)
- **권장 배속 1.3** — 빠른 숏폼 톤. 무음 압축(3단계 자동) 후 실측
  **0.167초/음절**이라, 완성 30초를 노리면 대본 **145~165음절**로 잡는다
  (나레이션 약 25초 + 클립 여백 12~17%). 압축 전 기준이던 0.183초/음절은
  침묵까지 센 값이라 더는 쓰지 않는다
...
| `프롬프트_자동생성용.txt` | 조립된 생성 프롬프트 (2,400자) |
| `프롬프트_직접생성용_텍스트만.txt` | 레퍼런스 미첨부용 (헤더 추가본) |
| `프롬프트_직접생성용_레퍼런스첨부.txt` | 레퍼런스 첨부용 (소재·로고 복제 금지 문단 추가) |
...
| 항목 | 채택본 (**텍스트 프롬프트만**) | alt (**레퍼런스 13장 첨부**) |
|---|---|---|
| 인물 렌더 | 사실적, 원본에 근접 | 카툰/스타일라이즈드 |
| 소품 | 구체적 (드로퍼 병) | 추상적 (구슬·물컵) |
| 팔레트 스와치 | 5색 모두 식별됨 | 첫 색(파랑)이 배경에 묻힘 |
| 오탈자 | 없음 | "SKIN MAYERS" (LAYERS 오타) |

### ⚠️ 예상과 반대로 나온 결과 (2026-08-01, 사용자 확인)

**레퍼런스를 첨부한 쪽이 오히려 원본에서 멀어졌다.** 첨부 없이 11슬롯 텍스트
프롬프트만 쓴 쪽이 사실적 렌더·정확한 팔레트·무오타로 더 나았다.

추정 원인: 서로 다른 장면 13장을 첨부하면 모델이 그것들을 평균내려다
일반화되고, 정작 hex·재질·조명을 **문장으로 못 박은 텍스트**가 더 강한
지시가 된다. 레퍼런스가 노이즈로 작용한 셈.

**함의**: 스타일 시트를 렌더할 때는 **텍스트 프롬프트 단독을 기본으로 한다.**
레퍼런스 첨부는 텍스트만으로 질감이 안 잡힐 때의 보조 수단으로만 쓴다.
(표본 1회이므로 다음 스타일에서 재확인 필요)
